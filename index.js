...
// ─── Save / reset helpers ─────────────────────────────────────────────────────
// Returns true if a save is earned at this count value
function earnsSave(value) {
    const milestones = [50, 100, 250, 500, 1000, 2000];
    if (milestones.includes(value)) return true;
    return value > 2000 && value % 2000 === 0;
}
...
client.on('messageCreate', async message => {
    ...
    if (state.countType === 'random') {
        ...
        const earnedSave = earnsSave(value);
        if (earnedSave) {
